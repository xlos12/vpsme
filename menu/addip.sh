...
adminip=$( curl -sS https://raw.githubusercontent.com/xlos12/allow/main/accessmenu | grep $MYIP )
...
setadmin() {
#isi link git
linkinstall="https://raw.githubusercontent.com/xlos12/allow/main"
rm -rf /etc/adminip/
mkdir -p /etc/adminip
...
sleep 1
cek=$( curl -sS https://raw.githubusercontent.com/xlos12/allow/main/ipvps.conf | awk '{print $5}' | grep $daftar )
if [[ $daftar = $cek ]]; then
...
git config --global user.name "xlos12"
git clone https://github.com/xlos12/allow.git
mkdir /root/allow
cd /root/allow/
rm -rf .git
git init
...
git branch -M main
git remote add origin https://github.com/xlos12/allow.git
git push -f https://${tokengit}@github.com/xlos12/allow.git
